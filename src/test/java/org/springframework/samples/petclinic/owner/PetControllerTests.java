...
class PetControllerTests {
	...
	private static final String PET_BIRTH_DATE_PARAM = "birthDate";
	private static final String PET_BIRTH_DATE = "2015-02-12";
	private static final String OWNER_MODEL_ATTR = "owner";
	...
	void testProcessCreationFormSuccess() throws Exception {
		mockMvc
			.perform(post(NEW_PET_URL, TEST_OWNER_ID).param("name", PET_NAME_BETTY)
				.param("type", PET_TYPE_HAMSTER)
				.param(PET_BIRTH_DATE_PARAM, PET_BIRTH_DATE))
	...
	class ProcessCreationFormHasErrors {
		...
		void testProcessCreationFormWithBlankName() throws Exception {
			mockMvc
				.perform(post(NEW_PET_URL, TEST_OWNER_ID).param("name", "\t \n")
					.param(PET_BIRTH_DATE_PARAM, PET_BIRTH_DATE))
				.andExpect(model().attributeHasNoErrors(OWNER_MODEL_ATTR))
		...
		void testProcessCreationFormWithInvalidBirthDate() throws Exception {
			...
			mockMvc
				.perform(post(NEW_PET_URL, TEST_OWNER_ID).param("name", PET_NAME_BETTY)
					.param(PET_BIRTH_DATE_PARAM, futureBirthDate))
				.andExpect(model().attributeHasNoErrors(OWNER_MODEL_ATTR))
				.andExpect(model().attributeHasErrors("pet"))
				.andExpect(model().attributeHasFieldErrors("pet", PET_BIRTH_DATE_PARAM))
				.andExpect(model().attributeHasFieldErrorCode("pet", PET_BIRTH_DATE_PARAM, "typeMismatch.birthDate"))
				.andExpect(status().isOk())
				.andExpect(view().name(PET_FORM_VIEW));
		}
	...
	void testProcessUpdateFormSuccess() throws Exception {
		mockMvc
			.perform(post(EDIT_PET_URL, TEST_OWNER_ID, TEST_PET_ID).param("name", PET_NAME_BETTY)
				.param("type", PET_TYPE_HAMSTER)
				.param(PET_BIRTH_DATE_PARAM, PET_BIRTH_DATE))
			.andExpect(status().is3xxRedirection())
			.andExpect(view().name("redirect:/owners/{ownerId}"));
		...
		void testProcessUpdateFormWithInvalidBirthDate() throws Exception {
			mockMvc
				.perform(post(EDIT_PET_URL, TEST_OWNER_ID, TEST_PET_ID).param("name", " ")
					.param(PET_BIRTH_DATE_PARAM, "2015/02/12"))
				.andExpect(model().attributeHasNoErrors(OWNER_MODEL_ATTR))
				.andExpect(model().attributeHasErrors("pet"))
				.andExpect(model().attributeHasFieldErrors("pet", PET_BIRTH_DATE_PARAM))
				.andExpect(model().attributeHasFieldErrorCode("pet", PET_BIRTH_DATE_PARAM, "typeMismatch"))
				.andExpect(view().name(PET_FORM_VIEW));
		}
