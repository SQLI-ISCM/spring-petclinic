...
class PetControllerTests {
	...
	private static final String OWNERS_OWNERID_PETS_NEW = "/owners/{ownerId}/pets/new";
	private static final String OWNERS_OWNERID_PETS_PETID_EDIT = "/owners/{ownerId}/pets/{petId}/edit";
	...
	void testInitCreationForm() throws Exception {
		mockMvc.perform(get(OWNERS_OWNERID_PETS_NEW, TEST_OWNER_ID))
	...
	void testProcessCreationFormSuccess() throws Exception {
		mockMvc
			.perform(post(OWNERS_OWNERID_PETS_NEW, TEST_OWNER_ID).param("name", BETTY)
				.param("type", HAMSTER)
	...
	class ProcessCreationFormHasErrors {
		...
		void testProcessCreationFormWithBlankName() throws Exception {
			mockMvc
				.perform(post(OWNERS_OWNERID_PETS_NEW, TEST_OWNER_ID).param("name", "\t \n")
					.param(BIRTH_DATE, DATE_2015_02_12))
		...
		void testProcessCreationFormWithDuplicateName() throws Exception {
			mockMvc
				.perform(post(OWNERS_OWNERID_PETS_NEW, TEST_OWNER_ID).param("name", "petty")
					.param(BIRTH_DATE, DATE_2015_02_12))
				.andExpect(model().attributeHasNoErrors(OWNER))
				.andExpect(model().attributeHasErrors("pet"))
				.andExpect(model().attributeHasFieldErrors("pet", "name"))
		...
		void testInitUpdateForm() throws Exception {
			mockMvc.perform(get(OWNERS_OWNERID_PETS_PETID_EDIT, TEST_OWNER_ID, TEST_PET_ID))
				.andExpect(status().isOk())
				.andExpect(model().attributeExists("pet"))
				.andExpect(view().name(PETS_CREATE_OR_UPDATE_PET_FORM));
		}
	...
	void testProcessUpdateFormSuccess() throws Exception {
		mockMvc
			.perform(post(OWNERS_OWNERID_PETS_PETID_EDIT, TEST_OWNER_ID, TEST_PET_ID).param("name", BETTY)
				.param("type", HAMSTER)
				.param(BIRTH_DATE, DATE_2015_02_12))
			.andExpect(status().is3xxRedirection())
			.andExpect(view().name("redirect:/owners/{ownerId}"));
	...
	class ProcessUpdateFormHasErrors {
		...
		void testProcessUpdateFormWithInvalidBirthDate() throws Exception {
			mockMvc
				.perform(post(OWNERS_OWNERID_PETS_PETID_EDIT, TEST_OWNER_ID, TEST_PET_ID).param("name", " ")
