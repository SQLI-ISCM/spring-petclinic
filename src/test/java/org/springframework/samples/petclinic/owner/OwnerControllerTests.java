...
class OwnerControllerTests {
	...
	private static final String OWNERS_LIST_PAGE = "/owners?page=1";
	...
	void testProcessCreationFormSuccess() throws Exception {
		mockMvc
			.perform(post(NEW_OWNER_URL).param(FIRST_NAME, "Joe")
	...
	void testProcessCreationFormHasErrors() throws Exception {
		mockMvc
			.perform(post(NEW_OWNER_URL).param(FIRST_NAME, "Joe").param(LAST_NAME, BLOGGS).param("city", LONDON))
			.andExpect(status().isOk())
	...
	void testProcessFindFormSuccess() throws Exception {
		...
		mockMvc.perform(get(OWNERS_LIST_PAGE)).andExpect(status().isOk()).andExpect(view().name("owners/ownersList"));
	}
	...
	void testProcessFindFormByLastName() throws Exception {
		...
		mockMvc.perform(get(OWNERS_LIST_PAGE).param(LAST_NAME, FRANKLIN))
			.andExpect(status().is3xxRedirection())
	...
	void testProcessFindFormNoOwnersFound() throws Exception {
		...
		mockMvc.perform(get(OWNERS_LIST_PAGE).param(LAST_NAME, "Unknown Surname"))
			.andExpect(status().isOk())
	...
	void testProcessUpdateOwnerFormSuccess() throws Exception {
		mockMvc
			.perform(post(EDIT_OWNER_URL, TEST_OWNER_ID).param(FIRST_NAME, "Joe")
				.param(LAST_NAME, BLOGGS)
				.param(ADDRESS, "123 Caramel Street")
				.param("city", LONDON)
